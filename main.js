...
function handleSlidebtnclick(e) {
  // TODO: check if slider is already hidden
  e.currentTarget.id === "prev" ? currentIndex-- : currentIndex++;
  slidecontainer.dispatchEvent(new Event("sliderMove"));
}
//remove/add att fx
const removeDisabledatt=(els)=> els.forEach(el=>el.removeAttribute('disabled'));
const addDisabledatt=(els)=> els.forEach(el=>el.setAttribute('disabled', 'true'));
...
slidecontainer.addEventListener("sliderMove", () => {
  console.log("moved");
  //moving container to right/left
  slidecontainer.style.transform = `translateX(-${currentIndex*slides[0].clientWidth}px)`;
  removeDisabledatt(slidebtns);
  currentIndex=== 0 && addDisabledatt([slidebtns[0]]);
  currentIndex=== slides.length-1 && addDisabledatt([slidebtns[1]]);
})
//slider intersection observer
 const sliderobs= new IntersectionObserver((slide)=>{
  if(slide[0].isIntersecting){
    console.log()
    addDisabledatt([slidebtns[1]]);
  }
 },{thershold: .75});

  sliderobs.observe(slides[(slides.length)-1]);
